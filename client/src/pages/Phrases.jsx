    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-stone-400 text-sm font-sans">{rows.length} phrases saved</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm font-sans">
            <thead>
              <tr className="border-b border-stone-800">
                {['Phrase', 'English Meaning', 'Level', 'Source', 'Date Added'].map(h => (
            ...
            <tbody>
              {filtered.map((row, i) => (
                <tr key={i} className="border-b border-stone-800/50 hover:bg-stone-800/40 transition-colors">
                  <td className="px-4 py-3 font-semibold text-stone-100 italic">{row.Phrase}</td>
                  <td className="px-4 py-3 text-stone-300">{row['English Meaning']}</td>
                  ...
                  <td className="px-4 py-3 text-stone-600 text-xs">{row['Date Added']}</td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-stone-600">No phrases found.</td>
                </tr>
              )}
            </tbody>
          </table>
